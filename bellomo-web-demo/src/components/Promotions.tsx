...
export default function Promotions() {
  ...
  return <section id="promociones" className="bellomo-promotions" aria-labelledby="promotions-title">
    <div className="promotions-inner">
      <header className="promotions-heading"><div><p className="promotions-kicker">BELLOMO · OPORTUNIDADES</p><h2 id="promotions-title" className="font-display">Un próximo paso,<br/>más cerca.</h2></div><p>Conocé nuestras propuestas y consultá sus condiciones con el equipo de Bellomo.</p></header>
      <div className="promotions-list">{promotions.map((item,index)=><article key={item.id} className={`promotion-card ${index===0?'promotion-featured':''}`}>
        <div className="promotion-photo"><img src={item.image || '/images/demo-generated/casa.webp'} alt={item.title} loading="lazy"/><span>PROPUESTA {String(index+1).padStart(2,'0')}</span></div>
