// Condiciones comerciales transcriptas de Financiación Bellomo.
// Interés mensual sobre el capital inicial financiado confirmado por el equipo comercial.
...
export function findFinancingPlan(name?: string): FinancingPlan | undefined {
  if (!name) return undefined;
  const normalize = (value: string) => value.normalize('NFD').replace(/[\u0300-\u036f]/g, '').trim().toLowerCase();
  const aliases: Record<string, string> = {
    'loteo san pablo 1': 'san pablo 1',
    'san pablo 1': 'san pablo 1',
    'san pablo': 'san pablo 1',
    'valle verde loteo': 'valle verde',
    'valle verde': 'valle verde',
    'remanente el rocio': 'el rocio',
    'el rocio': 'el rocio',
    'el rocío': 'el rocio',
    'remanente el rocío': 'el rocio',
  };
  const normalized = normalize(name);
  const targetKey = aliases[normalized] ?? normalized;
  return FINANCING_PLANS.find(plan => normalize(plan.name) === targetKey);
}
...
export function financingSummary(
  ...
  const minimum = 'percent' in plan.minimum
    ? (net * plan.minimum.percent) / 100
    : currency === 'ARS'
    ? plan.minimum.ars
    : Number.isFinite(exchange) && exchange > 0
    ? plan.minimum.ars / exchange
    : null;
  ...
  return {
    ...
    initial,
    valid,
    balance: valid && initial !== null ? net - initial : null
  };
}

export function cacPayment(balance: number, months: number, baseIndex: number, dueIndex: number) {
  if (
    ![balance, months, baseIndex, dueIndex].every(Number.isFinite) ||
    balance < 0 ||
...
export function fixedPayment(balance: number, months: number, monthlyPercent: number) {
  if (
    ![balance, months, monthlyPercent].every(Number.isFinite) ||
    balance < 0 ||
    !Number.isInteger(months) ||
    months < 1 ||
    monthlyPercent < 0
