...
$mode = $argv[1] ?? '--preview';
if (! in_array($mode, ['--preview', '--apply', '--restore'], true)) {
    exit(1);
}
try {
    DB::transaction(function () use ($mode) {
        ...
        if (! $property || $property->title !== 'PRUEBA QA · Casa de validación 14-09'
            || $property->description !== 'Registro ficticio de QA local. No comercializar.') {
            throw new RuntimeException('QA identity mismatch.');
        }
        ...
        if ($mode === '--apply') {
            ...
            if ($linked) {
                throw new RuntimeException('Active lead references this property; aborting.');
            }
            $backup = ['database' => DB::connection()->getDatabaseName(), 'marker' => now()->format('Y-m-d H:i:s.v'), 'row' => $property];
            $file = @fopen($path, 'x');
            if (! $file) {
                throw new RuntimeException('Backup exists or is unavailable.');
            }
            ...
            if ($backup['database'] !== DB::connection()->getDatabaseName() || $backup['row']['id'] !== $property->id
                ...
                || $property->updated_at !== $backup['row']['updated_at']) {
                throw new RuntimeException('Record changed; restore aborted.');
            }
            DB::table('properties')->where('tenant_id', 1)->where('id', $property->id)
