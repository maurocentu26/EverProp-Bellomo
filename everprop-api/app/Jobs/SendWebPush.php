...
final class SendWebPush implements ShouldQueue
{
    ...
    public function __construct(public int $tenantId, public int $userId, public string $notificationId) { $this->afterCommit(); }
    public function handle(): void {
        if (!config('webpush.private_key') || !config('webpush.subject')) return;
        $user = User::where('tenant_id', $this->tenantId)->where('id', $this->userId)->where('status', 'ACTIVE')->first();
        if (!$user || !(new \App\Domain\Identity\Policies\WebPushPolicy)->manage($user, $this->tenantId)) return;
        $tenant = \App\Domain\Tenancy\Models\Tenant::find($this->tenantId);
        if (!$tenant) return;
        $context = \App\Domain\Tenancy\TenantContext::forTenant($tenant);
        ...
        foreach ($rows as $row) {
            ...
            if ($report->isSubscriptionExpired()) DB::table('web_push_subscriptions')->where('tenant_id', $context->id())->where('user_id', $user->id)->where('id', $row->id)->delete();
            elseif (!$report->isSuccess()) throw new \RuntimeException('El proveedor push no confirmó la entrega.');
        }
    }
}
