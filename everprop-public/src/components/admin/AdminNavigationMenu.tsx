...
export function AdminNavigationMenu({
  ...
  const quickActions = useMemo(
    () => getAvailableQuickActions({ isEngineer, isAdvisor }).filter((action) => {
      if (action.href === "/admin/agenda" && !isMockDataMode) return false;
      if (action.href.endsWith("/new") && !canCreate) return false;
      return true;
    }),
    [canCreate, isEngineer, isAdvisor],
  );
